...
function InsertarLog($Type, $Code, $Consulta){
	...
	if(isset($_SESSION['CodUser'])&&($_SESSION['CodUser']!="")){
		...
		else
		{
			...
			if($cod_http!=200){//Ocurrio un error
				...
				if(!isset($result->Success)){
					$json_array=array(
						"Success" => 0,
						"Mensaje" => "Error $cod_http. Ha ocurrido un error en la conexión. Por favor verificar el servicio."
					);
...
function AuthJWT($pUser, $pPassword, $pEndpoint='Login', $pParametros=''){
	...
	if($cod_http!=200){//Ocurrio un error
		if(!isset($result->Success)){
			$Success=0;
			$token="Error $cod_http. Ha ocurrido un error en la conexión. Por favor verificar el servicio.";
		}else{
			$Success=0;
