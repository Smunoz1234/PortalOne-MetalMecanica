...
if(isset($_GET['dt_OF'])&&($_GET['dt_OF'])==1){//Verificar que viene de una Oferta de compras
	...
	$SQL_CopiarOfertaToOrden=EjecutarSP('sp_tbl_OfertaVentaDet_To_OrdenCompraDet',$ParametrosCopiarOfertaToOrden);
...
function BuscarArticulo(dato){
	...
	if(dato!=""){
		if((cardcode!="")&&(almacen!="")){
			remote=open('buscar_articulo.php?dato='+dato+'&cardcode='+cardcode+'&whscode='+almacen+'&doctype=<?php if($edit==0){echo "1";}else{echo "2";}?>&idordencompra=<?php if($edit==1){echo base64_encode($row['ID_OrdenCompra']);}else{echo "0";}?>&evento=<?php if($edit==1){echo base64_encode($row['IdEvento']);}else{echo "0";}?>&tipodoc=2','remote',"width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left="+posicion_x+",top="+posicion_y+"");
			remote.focus();
		}else{
			Swal.fire({
				title: "¡Advertencia!",
					...
					<div class="col-lg-3">
						<div class="btn-group pull-right">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="alkin dropdown-item" href="entrada_compra.php?dt_OV=1&Cardcode=<?php echo base64_encode($row['CardCode']);?>&CCosto=<?php echo base64_encode($row['OcrCode2']);?>&Area=<?php echo base64_encode($row['OcrCode']);?>&Sucursal=<?php echo base64_encode($row['SucursalDestino']);?>&Direccion=<?php echo base64_encode($row['DireccionDestino']);?>&Almacen=<?php echo base64_encode($row['WhsCode']);?>&Contacto=<?php echo base64_encode($row['CodigoContacto']);?>&OV=<?php echo base64_encode($row['ID_OrdenCompra']);?>&Evento=<?php echo base64_encode($row['IdEvento']);?>&dt_LS=1&LS=<?php echo base64_encode($row['ID_LlamadaServicio']);?>">Entrada de compra</a></li>
