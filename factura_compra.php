...
if(isset($_GET['dt_OV'])&&($_GET['dt_OV'])==1){//Verificar que viene de una Orden de ventas
	...
	$SQL_CopiarOrdenToFactura=EjecutarSP('sp_tbl_OrdenVentaDet_To_FacturaCompraDet',$ParametrosCopiarOrdenToFactura);
...
function BuscarArticulo(dato){
	...
	if(dato!=""){
		if((cardcode!="")&&(almacen!="")){
			remote=open('buscar_articulo.php?dato='+dato+'&cardcode='+cardcode+'&whscode='+almacen+'&doctype=<?php if($edit==0){echo "1";}else{echo "2";}?>&idfacturacompra=<?php if($edit==1){echo base64_encode($row['ID_FacturaCompra']);}else{echo "0";}?>&evento=<?php if($edit==1){echo base64_encode($row['IdEvento']);}else{echo "0";}?>&tipodoc=2','remote',"width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left="+posicion_x+",top="+posicion_y+"");
			remote.focus();
		}else{
			Swal.fire({
				title: "¡Advertencia!",
...
<script type="text/javascript">
	$(document).ready(function() {//Cargar los combos dependiendo de otros
		$("#CardCode").change(function(){
			...
			<?php }else{?>
				if(carcode!="" && almacen!=""){
					frame.src="detalle_factura_compra.php?id=<?php echo base64_encode($row['ID_FacturaCompra']);?>&evento=<?php echo base64_encode($row['IdEvento']);?>&type=2";
				}else{
					frame.src="detalle_factura_compra.php";
				}
			<?php }?>		
