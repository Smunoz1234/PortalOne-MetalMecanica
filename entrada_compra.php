...
if(isset($_POST['P'])&&($_POST['P']!="")){//Grabar Entrada de compras
	...
	try{
		...
		if($SQL_CabeceraEntradaCompra){
			...
			try{
//				$Parametros=array(
//					'pIdEntradaCompra' => $IdEntradaCompra,
//					'pIdEvento' => $IdEvento,
//					'pLogin'=>$_SESSION['User']
//				);
//				$Metodo="AppPortal_InsertarEntradaCompra";
//				$Resultado=EnviarWebServiceSAP($Metodo,$Parametros);
				...
				if($Resultado->Success==0){
					...
					if($_POST['tl']==0){//Creando Entrada	
						...
						$SQL_ConsID=Seleccionar('uvw_Sap_tbl_EntradasVentas','ID_EntradaCompra',"IdDocPortal='".$IdEntradaCompra."'");
...
if(isset($_GET['dt_OV'])&&($_GET['dt_OV'])==1){//Verificar que viene de una Orden de compras
	...
	//Sucursal destino
	$SQL_SucursalDestino=Seleccionar('uvw_Sap_tbl_Proveedores_Sucursales','*',"CodigoCliente='".base64_decode($_GET['Cardcode'])."' AND NombreSucursal='".base64_decode($_GET['Sucursal'])."' and TipoDireccion='S'");
...
function BuscarArticulo(dato){
	...
	if(dato!=""){
		if((cardcode!="")&&(almacen!="")){
			remote=open('buscar_articulo.php?dato='+dato+'&cardcode='+cardcode+'&whscode='+almacen+'&doctype=<?php if($edit==0){echo "5";}else{echo "6";}?>&identregacompra=<?php if($edit==1){echo base64_encode($row['ID_EntradaCompra']);}else{echo "0";}?>&evento=<?php if($edit==1){echo base64_encode($row['IdEvento']);}else{echo "0";}?>&tipodoc=2','remote',"width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left="+posicion_x+",top="+posicion_y+"");
			remote.focus();
		}else{
			Swal.fire({
				title: "¡Advertencia!",
					...
					<div class="form-group">
						...
						<div class="col-lg-6">
							<?php if($row['DocDestinoDocEntry']!=""){?>
								<a href="factura_compra.php?id=<?php echo base64_encode($row['DocDestinoDocEntry']);?>&id_portal=<?php echo base64_encode($row['DocDestinoIdPortal']);?>&tl=1" target="_blank" class="btn btn-outline btn-success pull-right m-l-sm">Ir a documento destino <i class="fa fa-mail-forward"></i></a>
							<?php }?>
           ...
           <div class="col-lg-12">
              <form action="entrada_compra.php" method="post" class="form-horizontal" enctype="multipart/form-data" id="CrearEntradaCompra">
				...
				<div class="tabs-container">  
					...
					<div class="tab-content">
						<div id="tab-1" class="tab-pane active">
							<iframe id="DataGrid" name="DataGrid" style="border: 0;" width="100%" height="300" src="<?php if($edit==0){echo "detalle_entrada_compra.php";}else{echo "detalle_entrada_compra.php?id=".base64_encode($row['ID_EntradaCompra'])."&evento=".base64_encode($row['IdEvento'])."&docentry=".base64_encode($row['DocEntry'])."&type=2&status=".base64_encode($row['Cod_Estado']);}?>"></iframe>
						</div>
						<?php if($edit==1){?>
			   ...
			   <form id="frm" action="" class="form-horizontal">
				...
				<div class="col-lg-8">
					<div class="form-group">
						<label class="col-lg-2">Encargado de compras</label>
						<div class="col-lg-5">
					...
					<div class="form-group">
						...
						<div class="col-lg-10">
							<textarea name="Comentarios" form="CrearEntradaCompra" rows="4" class="form-control" id="Comentarios" <?php if(($edit==1)&&($row['Cod_Estado']=='C')){echo "readonly";}?>><?php if($edit==1){echo $row['Comentarios'];}?></textarea>
						</div>
					</div>
					<div class="form-group">
